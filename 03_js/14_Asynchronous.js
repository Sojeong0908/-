/* 
    동기(Synchronous)와 비동기(Asynchronous)
        - 동기적 : 결과값을 기다린다. cf.순서대로 처리 되는 것은 동기라고 생각하자. 
        - 비동기적 : 결과값을 기다리지 않는다. 
            => 비동기 처리를 위해서는 콜백함수로 처리하는 것이 가장 기본적이다. 
...
/* 
    프로미스(Promise)
        - 비동기 처리를 쉽게 도와주는 객체.
        - 객체 생성시 함수를 전달 받는데 그 함수의 첫번째 인자는 resolve함수, 두번째는 reject
        - resolve는 성공했을 때 실행할 함수
            => resolve가 된 상태를 fulfilled
            => Promise.prototype.then(콜백함수) : resolve된 이후에 실행된다. 
            => 콜백함수의 첫번째 인자로 resolve된 값을 전달한다.

        -reject : 실패했을 때 실행할 함수. 
            => reject("이유") 에러를 발생시키니다.
            => Promise.prototype.catch(콜백함수) : reject된 이후에 실행한다. 
...
/* 
  1. input 태그값 가져오기 => form태그의 submit 이벤트  발생했을 때 
...
/* 
    async/await
        - 프로미스를 동기적으로 처리하는 함수. 
        - async 함수 내부에서 Promise를 반환하는 함수 앞에 await를 붙이면,
          resolve되기 전까지 이후의 코드를 실행하지 않는다. 
        - promise를 처리할 때 동기적으로 볼 수 있게 작성할 수 있어서 읽기가 편하다. 
        - async 함수에서 값을 return 하면 return 값을 resolve하는 Promise가 변환된다
*/
...
async function renderMovie02(title) {
  ...
  try {
    ...
    // try/catch(e) : try 문의 코드를 실행하다 에러가 발생하면 catch문이 실행된다.
    alert(e);
  }
}
