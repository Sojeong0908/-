...
console.log(20 * 3);
console.log(20 / 4);
console.log(20 % 3); // 나누고 남는 나머지를 구하는 연산자.
console.log(20 - 3 / 3); // 사칙연산 고려해야 함.
...
// 후위 연산자 : 단항 연산자가 뒤에 붙으면 우선 순위가 마지막이 된다.
// 전위 연산자 : 단항 연산자가 앞에 붙으면 우선 순위가 가장 먼저다.
...
num03 += 30; //num 03(120)에 30을 더한 값을 num03에 다시 할당.
num03 /= 2;
...
// 부동등, 불일치 => 동등, 일치의 반대
console.log(1 != "1"); // 두 값이 같지 않은지 판단.
...
console.log(num04 >= 4); // num04가 4보다 크거나 같은지 => 크진 않지만 같기 때문에 false

/* 
    논리 연산자 : 두 논리값을 가지고 새로운 값. 
    ||(or) : 두 값 중 하나라도 true라면 true 반환 => 둘 다 false일 때만 false 반환
    &&(and) : 두 값 모두 true일 대만 true => 둘 중 하나라도 false라면 false 반환
    ! : 논리값을 반전시킨다. => true -> false, false -> true
*/

console.log(num04 > 10 || num05 >= 10); // true or false 이기 때문에 true
console.log(num04 < 10 && num05 > 4); // true and true 이기 때문에 true
console.log(!(num04 < 10)); // not true 이기 대문에 false

// 문자열 연결 연산자 : 좌항과 우항의 문자열을 연결
var str = "안녕하세요" + " 김소정입니다.";
console.log(str);
age = 27;
