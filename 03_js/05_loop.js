/* 
    ...
    for(초기화 변수; 조건식; 증감식)
        ...
        - 코드블록이 한번 실행이 끝날 때마다 증감식을 실행한다. 
            => 증감식이나 조건식이 잘못 작성된 경우 무한루프에 빠지게 된다.  

    while(조건식)
        - () 안의 조건식이 false가 될 때까지 반복적으로 실행한다.
        - 코드블록 안에서 종료시점을 제어해야한다. 
        - 반복횟수가 불명확할 때 사용한다. 
        - break문을 통해서 빠져나올 수 있다. (반복문 자체를 종료 <- continue와 비교)
...
for (var i = 2; i <= 9; i++) {
  console.log(i + "단 시작");
  for (var j = 1; j <= 9; j++) console.log("%d * %d =%d", i, j, i * j);
}
...
while (num03 < 10) {
  num03++;
  if (num03 % 2 !== 0) continue; // continue는 이번 반복만 건너 뛴다.
  console.log(num03);
}
