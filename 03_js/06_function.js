/* 
    ...
    매개변수(parameter)
        - 함수를 실행하는 데 필요한 값을 호출 시점에 전달 받는 변수. 
            => 함수 내부에서 사요하는 변수처럼 사용. 
            => 함수 내부에서만 유효한 범위를 가진다. 
        - 매개변수를 통해서 전달되 실제값을 '인자(argument)'라고 한다. 

    return 
        - 함수의 실행 결과로 값을 반환한다. 
            => 전달 받은 매개변수에 연산을 해서 값을 받고자 할 때 사용한다. 
            => 함수의 실행결과를 다른 함수의 인자로 사용하기도 한다. 
        - return은 함수의 종료를 의미하기도 한다. 
            => return 이후의 코드는 실행하지 않는다. 
*/

// 매개 변수가 없는 함수. => 기본적으로 반복해서 사용할 코드를 작성.
var num1 = 3;
var num2 = 4;
function sum() {
...
// 매개 변수가 있는 함수. (cf. a,b-> 매개변수, 10,5,7,9-> 인자) 반환값이 없는 함수.
function sum02(a, b) {
  console.log(a + b);
}
sum02(10, 5);
...
console.log(sum02(2, 3)); // return(반환값)이 있는 함수.
function sum03(a, b) {
  return a + b;
}
...
console.log(square(3)); // 함수 호이스팅이 발생. square 함수 선언문으로 작성했기 때문에 실행도 가능하다.
function square(num) {
  return num * num;
}
...
*/
foo(); // foo함수는 함수 선언문으로 작성했기 때문에 호이스팅 발생.
function foo() {
  console.log("foo 함수 실행");
}

//function의 이름이 없음. (위의 기명함수는 function 'foo'라는 함수의 이름이 있음)
let bar = function () {
...
/* 
  콜백함수(callback)
    - 다른 함수에 인자로 전달되는 함수 
      => 함수의 실행결과를 가지고 다른 함수를 실행 해야할 때.

    - JS에서는 함수를 값으로 평가하기 때문에 가능하다. 
      => 콜백함수로써 값을 전달할 때는 호출하는 형태가 되면 안된다. 함수는 값으로 취급하기 때문에 가능. 
      => 콜백함수를 호출해야 하는 경우 익명 함수로 감싸서 그 안에서 호출하면 된다. 
      => 콜백함수를 직접 호출해야되는 경우 익명 함수로 감싸서 그 안에서 호출하면 된다. 
...
// 인자 n을 전달받아서 콜백함수를 n번 반복 실행하는 함수 만들기.
...
/*  sum03사용해서 숫자 2개를 더한 값을 7번 출력하기 
...
/* 
  화살표 함수
    - 화살표를 통해서 함수를 간단하게 표현할 수 있다. 
      ...
      => 객체 return시 ()로 감싸주어야 return을 생략할 수 있다. 
    - 함수 호이스팅이 일어나지 않는다. 
    - this 바인딩이 되지 않는다. 
*/
console.log("---------화살표 함수---------");
...
let getUser = (name, age) => ({ name: name, age: age }); //()로 감싸서 객체임을 알려준다.
