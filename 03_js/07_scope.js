/* 
    유효 범위(scope)
        전역 스코프
            - 어디서든 접근 가능한 범위
            - JS 기본적으로 여러개 파일이 하나로 동작하기 위해 같은 전역 스코프를 가진다. 

        지역 스코프
            - 조건문, 반복문, 함수 등의 지역 내에서만 유효한 범위
            - 현재 스코프에서 없는 식별자를 참조하면 상위 스코프에서 찾는다. 전역스코프까지 찾아서 없으면 그때 오류 발생. 
                => 스코프 체인
...
//console.log(a); addGlobalNum의 매개 변수 a는 함수 내부에서만 유효하기 때문에 밖에서는 접근할 수 없다.
