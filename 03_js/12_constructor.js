/* 
    생성자(constructor)
      ...
      - 객체에 대한 형태를 미리 만들어 놓고 찍어서 생산할 수 있는 틀과 같은 개념.
      - 제품을 생산하기 위한 설계도와 같은 역할. 
      - 생성자 함수는 대문자로 시작한다. (파스칼 케이스)

    인스턴스(instance)
      - 실제로 메모리에 할당되는 생성자에 의해 생성된 객체.
...
/* 
  함수 내부에서 this를 참조하면 생성자 함수가 된다. 
    => this는 생성자 함수를 통해 생성될 인스턴스를 가리킨다. 
...
//          => 공통적으로 제공하는 기능을 만들 대 주로 사용.
User.isUser = function (user) {
  // user가 생성자 User로 만들어진 인스턴스가 맞는지 확인.
  console.log(user instanceof User);
};

const user03 = new User("seok", 31);
let user04 = new User("정철", 28);
...
/* 
  클래스(class)
    ...
    - class 키워드와 함께 사용한다. 
      => () 가 붙지 않는다. 
    - constructor를 통해 값을 초기화 할 수 있다. 
      => 초기화 할 값이 없다면 constructor를 생략 가능하다. 
      => 생략을 할 경우, JS가 내부적으로 자동으로 constructor를 실행한다. => 빈 객체를 만들어낸다. 
