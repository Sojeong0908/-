/* 
    ...
    전역 객체
        - JS가 실행되는 환경에서 가장 먼저 만들어지는 특수한 객체
            => 실행되는 환경에 따라서 전역 객체가 달라진다. 브라우저에서는 Window, Node.js에서는 global

        - 어디서든 참조할 수가 있는 최상위 객체(프로토타입 얘기X, 상속과는 별개의 개념.)
...
/*
     Date : 날짜와 시간에 대한 프로퍼티 및 메서드 제공.
        - 날짜에 대한 기준은 UTC 1970년 1월 1일 자정부터 흐른 시간을 밀리초 단위로 계산한다. 
 */
let today = new Date(); //  날짜에 대한 정보를 가지고 있는 인스턴스를 생성.
console.log("today: ", today); // 인스턴스를 출력하면 현재 시간에 대한 정보 출력.

let year = today.getFullYear(); // 날짜의 연도.
let month = today.getMonth(); // 월을 반환 1월(0) ~ 12월(11), tip. 0부터 배열하기 때문에 +1
let date = today.getDate(); // 일을 반환 (1 ~ 31)

let day = today.getDay(); // 요일을 반환 일요일(0) ~ 토요일(6), 0부터 배열함
let hours = today.getHours(); //시간 반환
let minutes = today.getMinutes(); // 분 반환
let seconds = today.getSeconds(); // 초 반호나

console.log(
  `${year}년 ${
    month + 1
  }월 ${date}일 ${day} ${hours}시 ${minutes}분 ${seconds}초`
);
...
yesterday.setDate(today.getDate() - 1); // 오늘 날짜를 기준으로 할 뺀 날로 설정.
...
console.log(ceilNum); //소수점 이하의 숫자 올림.

let roundNum = Math.round(floatNum); // 소수점 이하의 숫자 반올림.
console.log(roundNum);

let randomNum = Math.random(); // 0~1 사이의 랜덤한 실수.
...
//로또 번호 1~45의 숫자 6개 추첨하는 함수 만들어보기. => 중복허용X
// let lotto = [];
...
function getLottoNum() {
  let result = [];
  //result  배열의 개수가 6개가 될 때까지 반복 실행.
